...
Clone or download the repository from [github.com/kucher32/project-manifesto](https://github.com/kucher32/project-manifesto), copy its contents into your project folder, open the folder in Claude Code, and tell it: "read setup.md and install."
...
Another side effect: the Architect picks the Executor's model per step — Haiku for mechanical edits, Sonnet for typical work, Opus for reasoning depth, and Fable for long-horizon scope. The framework routes by complexity tier, so cheaper models run the mechanical majority. Token economy follows from the discipline, not from manual cost-watching.
