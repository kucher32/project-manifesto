# setup.md — manifesto v0.7.2 installation specification

This file is the installation specification for the project-manifesto v0.7.2 framework. It describes the six-phase install procedure that an AI assistant (typically Claude Code) performs on the user's behalf to install the framework into a project directory.
...
Before performing any operation described below, the assistant must obtain explicit consent from the user. Print this confirmation prompt and wait for an affirmative reply:
    ...
    [33m?[0m project-manifesto v0.7.2 install — ready to start.
...
and produce its output directly. Do not describe what it does — run it.
    ...
    print(f"\033[32m✓\033[0m manifesto v0.7.2 installed in {PROJECT_PATH}")
    ...
    print(f"       git add -A")
    print(f'       git commit -m "chore(manifesto): install v0.7.2 in {SELECTED_MODE}"')
    print()
    if SELECTED_MODE == "mode-1-triad":
...
Execute the install commit:
    ...
    git add -A
    git commit -m "chore(manifesto): install v0.7.2 in $SELECTED_MODE mode"

Print:
    echo -e "\033[32m✓\033[0m committed: chore(manifesto): install v0.7.2 in $SELECTED_MODE mode"
