# setup.md — manifesto v0.9.0 installation specification

This file is the installation specification for the project-manifesto v0.9.0 framework. It describes the six-phase install procedure that an AI assistant (typically Claude Code) performs on the user's behalf to install the framework into a project directory.
...
Before performing any operation described below, the assistant must obtain explicit consent from the user. Print this confirmation prompt and wait for an affirmative reply:
    ...
    [33m?[0m project-manifesto v0.9.0 — ready to start.

    If this project has no manifesto yet, this is a fresh install. It adds:
    ...
    On a fresh install, your existing project files are not touched.

    If this project already has manifesto (a .manifesto-version file is present),
    this runs an update instead. An update refreshes manifesto config files that
    you have NOT modified, leaves files you HAVE modified untouched, and points
    you at a migration guide for the modified ones. Your own work and source
...
## Step 1b — Update detection

If a .manifesto-version file exists at the project root, this project already
has manifesto installed. In that case run update mode (this Step 1b) and SKIP
Steps 2 through 5 entirely (the fresh-install path). Step 6 (MCP) is also
skipped on update because MCP is already configured from the original install.

If .manifesto-version does NOT exist, skip this Step 1b and proceed to Step 2
as a fresh install.

Detection and update are performed by this Python snippet. Run it exactly:

    python3 << 'PYEOF'
    import hashlib, sys
    from pathlib import Path

    # DIST_VERSION must stay in sync with MANIFESTO_VERSION in Step 4.
    # Both are bumped together at release time.
    DIST_VERSION = "0.9.0"

    marker = Path(".manifesto-version")
    if not marker.exists():
        print("FRESH_INSTALL")
        sys.exit(0)

    lines = marker.read_text(encoding="utf-8").splitlines()
    installed_version = lines[0].strip() if lines else ""
    stored = {}
    for ln in lines[1:]:
        ln = ln.strip()
        if not ln:
    ...
    print(f"\033[36m→\033[0m update mode: installed {installed_version}, distribution {DIST_VERSION}")

    if installed_version == DIST_VERSION:
        print(f"\033[32m✓\033[0m already on {DIST_VERSION} — nothing to update")
        print("UPDATE_DONE")
        sys.exit(0)

    # Detect which mode is installed by which Category B files exist.
    if Path("MANIFESTO.md").exists() and Path("claude-desktop-prefs.md").exists():
        mode = "mode-1-triad"
        category_b = ["CLAUDE.md", "MANIFESTO.md", "claude-desktop-prefs.md", ".claude/instructions.md"]
    else:
        mode = "mode-2-solo"
        category_b = ["CLAUDE.md", ".claude/instructions.md"]
    print(f"\033[36m→\033[0m detected mode: {mode}")

    untouched = []
    modified = []
    for rel in category_b:
        p = Path(rel)
        if not p.exists():
            continue
        current = hashlib.sha256(p.read_bytes()).hexdigest()
        if stored.get(rel) == current:
            untouched.append(rel)
        else:
            modified.append(rel)

    print()
    print(f"\033[32m✓\033[0m unmodified (safe to auto-update): {len(untouched)}")
    for f in untouched:
        print(f"    {f}")
    print(f"\033[33m!\033[0m modified by you (will NOT be overwritten): {len(modified)}")
    for f in modified:
        print(f"    {f}")
    print()
    print("UPDATE_PLAN_READY")
    print(f"MODE={mode}")
...
If the output contains FRESH_INSTALL: skip the rest of Step 1b, go to Step 2.

If the output contains UPDATE_DONE: the project is already current. Print nothing
further, stop here. Do not proceed to Step 2 or beyond.

If the output contains UPDATE_PLAN_READY: continue with the apply phase below,
using MODE, UNTOUCHED, MODIFIED parsed from the output.

Apply phase. Run this snippet exactly, substituting the parsed values:
    ...
    DIST_VERSION = "0.9.0"
    MODE = "<MODE from detection output>"
    UNTOUCHED = "<UNTOUCHED from detection output>".split(",") if "<UNTOUCHED from detection output>" else []
    MODIFIED = "<MODIFIED from detection output>".split(",") if "<MODIFIED from detection output>" else []
    UNTOUCHED = [x for x in UNTOUCHED if x]
    MODIFIED = [x for x in MODIFIED if x]
    ...
    try:
        OPERATOR_NAME = subprocess.check_output(["git", "config", "user.name"], stderr=subprocess.DEVNULL).decode().strip()
    except subprocess.CalledProcessError:
        OPERATOR_NAME = ""
    ...
    template_map = {
        "CLAUDE.md": f".manifesto/{MODE}/CLAUDE.md.template",
        "MANIFESTO.md": ".manifesto/mode-1-triad/MANIFESTO.template.md",
        "claude-desktop-prefs.md": ".manifesto/mode-1-triad/claude-desktop-prefs.md.template",
        ".claude/instructions.md": f".manifesto/{MODE}/.claude/instructions.md.template",
    }

    # Auto-overwrite untouched files from the new templates.
    for rel in UNTOUCHED:
        ...
        if src and Path(src).exists():
            ...
            print(f"\033[32m✓\033[0m updated {rel} (was unmodified)")

    # Modified files: do not touch. Point at migration guide.
    if MODIFIED:
        print()
        print(f"\033[33m!\033[0m {len(MODIFIED)} file(s) you modified were left untouched:")
        for rel in MODIFIED:
            print(f"    {rel}")
        guide = f"docs/migrations/v{DIST_VERSION}.md"
        print(f"   To apply the new version's changes to these by hand, see the")
        print(f"   migration guide for this release (if provided in the distribution):")
        print(f"     {guide}")

    # Rewrite .manifesto-version with new version and fresh hashes of all Category B files.
    if MODE == "mode-1-triad":
        all_b = ["CLAUDE.md", "MANIFESTO.md", "claude-desktop-prefs.md", ".claude/instructions.md"]
    else:
        all_b = ["CLAUDE.md", ".claude/instructions.md"]
    ...
    Path(".manifesto-version").write_text("\n".join(out) + "\n", encoding="utf-8")
    print()
    print(f"\033[32m✓\033[0m .manifesto-version updated to {DIST_VERSION}")
    print("UPDATE_APPLIED")
    PYEOF

After the apply snippet prints UPDATE_APPLIED, refresh Category A and clean up.
The new .manifesto/ archive is already in place (the operator unpacked the new
distribution before running setup). Commit the update, then remove installer
artifacts as in fresh install:

    git add -A
    git commit -m "chore(manifesto): update to v0.9.0"
    rm -f setup.md LICENSE
    rm -rf .manifesto/
    git add -A
    git commit -m "chore: remove manifesto installer after update"
    echo -e "\033[32m✓\033[0m update complete — now on v0.9.0"

Update mode ends here. Do NOT proceed to Step 2 or any later step.
...
After the mode-specific install completes, write the version marker file
.manifesto-version at the project root. This records the installed manifesto
version and a sha256 hash of each Category B file as it exists on disk after
placeholder substitution. The update mode (D17) reads this file to detect which
files the adopter later modified.

Run this Python snippet exactly:

    python3 << 'PYEOF'
    import hashlib
    from pathlib import Path

    MANIFESTO_VERSION = "0.9.0"
    ...
    Path(".manifesto-version").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("\033[32m✓\033[0m wrote .manifesto-version (version + Category B hashes)")
    PYEOF
...
and produce its output directly. Do not describe what it does — run it.
    ...
    print(f"\033[32m✓\033[0m manifesto v0.9.0 installed in {PROJECT_PATH}")
    ...
    print(f"       git add -A")
    print(f'       git commit -m "chore(manifesto): install v0.9.0 in {SELECTED_MODE}"')
    print()
    if SELECTED_MODE == "mode-1-triad":
...
Execute the install commit:
    ...
    git add -A
    git commit -m "chore(manifesto): install v0.9.0 in $SELECTED_MODE mode"

Print:
    echo -e "\033[32m✓\033[0m committed: chore(manifesto): install v0.9.0 in $SELECTED_MODE mode"
